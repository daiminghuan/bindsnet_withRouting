...
# dataset = Route_data(csv_file='./route/TTC_train4.csv',
#                                     root_dir ='./route', time = time, dt = dt)
...
#     pbar.set_description_str("Train progress: (%d / %d)" % (i+1, n_iters))
#
...
#         #     get_square_weights(C1.w, 8, ), im=weights_im, wmin=-2, wmax=2
#         # )
...
model.load_state_dict(torch.load("8char.pth"))
...
for (i, dataPoint) in pbar:
    ...
    end_test1 = tm.time()
    print("coding time + network runing time ", end_test1 - start_test1)
    if plot:
...
for s, label, each_index in test_pairs:
    ...
    end_test = tm.time()
    print("end test, duration_time:", end_test - start_test)
    total += 1
